...
class SpeechTextJointToTextTask(SpeechToTextTask):
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument(
            "--test-model-type",
            default=None,
            choices=["text", "speech", "text_speech", "mt"],
        )
        parser.add_argument(
            "--load-text-valid-split", 
            default="train",
            type=str,
            help="load text data only",
        )        
        parser.add_argument(
            "--mask-text-type",
            default="random",
    ...
    def __init__(self, args, src_dict, tgt_dict, infer_tgt_lang_id=None):
        ...
        self.test_model_type = args.test_model_type
        self.load_text_valid_split = args.load_text_valid_split
    ...
    def load_langpair_dataset(
        self, prepend_tgt_lang_tag=False, sampling_alpha=1.0, epoch=0, split = "train"
    ):
        lang_pairs = []
        text_dataset = None
    ...
    def inference_step(
        ...
        sample["net_input"]["test"] = self.test_model_type
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        noise_token_id = -1
        text_dataset = None
        if self.args.parallel_text_data != "" and (is_train_split or self.test_model_type == "mt"):
            text_dataset = self.load_langpair_dataset(
                self.data_cfg.prepend_tgt_lang_tag_no_change, 1.0, epoch=epoch, split=self.load_text_valid_split
            )
        ...
        if text_dataset is not None and ( self.test_model_type != "mt") :
            mdsets = [
                ModalityDatasetItem(
                    "sup_speech",
                    ast_dataset,
