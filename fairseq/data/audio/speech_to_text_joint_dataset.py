...
class SpeechToTextJointDataset(SpeechToTextDataset):
    ...
    def __init__(
        ...
        self.src_langs = ['en'] * self.n_samples
        self.tgt_langs = ['de'] * self.n_samples
        self.asr = asr
    ...
    def get_tokenized_asr_text(self, index: int):
        text = self.tokenize(self.src_pre_tokenizer, self.asr[index])
        text = self.tokenize(self.src_bpe_tokenizer, text)
        return text
    ...
    def __getitem__(self, index: int) -> SpeechToTextJointDatasetItem:
        ...
        if self.src_texts is not None and self.src_dict is not None:
            ...
            if self.cfg.prepend_src_lang_tag:
                ...
                lang_src_idx = self.src_dict.index(lang_tag)
                src_tokens = torch.cat((torch.LongTensor([lang_src_idx]), src_tokens), 0)
        ...
        if self.cfg.prepend_tgt_lang_tag:
            ...
            s2t_dataset_item.target = torch.cat((torch.LongTensor([lang_tag_idx]), s2t_dataset_item.target), 0)
        ...
        if self.asr is not None:
            asr_tokens = self.get_tokenized_asr_text(index)
            asr_tokens = self.src_dict.encode_line(
                asr_tokens, add_if_not_exist=False, append_eos=True
            ).long()
    ...
    def collater(self, samples: List[SpeechToTextJointDatasetItem]) -> Dict:
        ...
        if self.src_texts is not None and self.src_dict is not None:
            ...
            if self.asr is not None:
                asr_target = fairseq_data_utils.collate_tokens(
                    [x.asr for x in samples],
                    self.tgt_dict.pad(),
                    self.tgt_dict.eos(),
                    left_pad=False,
                    move_eos_to_beginning=False,
                )
                asr_target = asr_target.index_select(0, order)
                asr_target_lengths = torch.tensor(
                    [x.asr.size(0) for x in samples], dtype=torch.long
                ).index_select(0, order)
                prev_output_asr_tokens = fairseq_data_utils.collate_tokens(
                    [x.asr for x in samples],
                    self.tgt_dict.pad(),
                    eos_idx=None,
                    left_pad=False,
                ...
                prev_output_asr_tokens = prev_output_asr_tokens.index_select(0, order)                
        ...
        out = {
            ...
            "prev_output_asr_tokens": prev_output_asr_tokens,
...
class SpeechToTextJointDatasetCreator(SpeechToTextDatasetCreator):
    ...
    def _from_list(
        ...
        asr = [s.get(cls.KEY_ASR, cls.DEFAULT_ASR) for s in samples]
